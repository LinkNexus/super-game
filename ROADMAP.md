...
| L57 | **Wire the sims' `removePlayer()` into the server's disconnect path** | `CoopGameSim::removePlayer`/`PvPGameSim::removePlayer` were written as part of L28 but **never reached**: `Game::removePlayer` frees the `PlayerConnection` and its server-side slot only, leaving the disconnected player's `PlayerSimState` in the sim — still drawn, still collidable, still counted alive. PvP is the severe case, since `isTeamDead` treats an occupied slot with lives > 0 as alive: one disconnect in a 1v1 strands the match in `PLAYERS_FIGHT` with no winner possible. Coop is milder (`allPlayersDead` can't trip while a ghost holds lives, and `players_count_` goes stale). **Attempted fix, still not effective**: a `std::visit([playerId](auto &s) { s.removePlayer(playerId); }, sim_)` now exists in `Game::removePlayer`, but it sits *after* the slot-search loop, which `return`s as soon as it finds the player — so the visit only runs on the path where there was nothing to remove. Structurally the same mistake as the `PvPGame::removePlayer` bug already fixed in L28: the cleanup is parked on the branch that can't reach it. Moving the visit inside the `if` before the `return` closes it (`playerId` is taken by value, so it's safe on either side of the `delete`). Worth a unit test rather than another read-through — this is the second time the same shape has slipped through. |
