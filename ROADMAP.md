...
| L23 | **Fix restart button routing on Game Over/Win** | `Game::handleInput()`'s `GAME_OVER`/`WIN` cases currently map both `KEY_ENTER` and `KEY_R` to the same `init()` call, which just resets to `Screen::MENU` — "R to restart" is a dead duplicate of ENTER. Local: restart should recreate a `LocalSession` directly with the already-persisted `selected_local_mode_` and jump straight to `Screen::PLAYING` (mirroring the `inputs_`/`player_id` setup the `SELECT_LOCAL_MODE` ENTER handler does), rather than forcing the mode to be reselected. Online: restart should skip mode reselection and go straight to `Screen::CONNECTING` with a fresh `OnlineSession`, landing back in the lobby instead of the main menu. |
| L24 | **Player display names in online mode** | `Player::draw()` currently hardcodes labels as `"Player " + (idx+1)` — no name concept exists anywhere in the wire format (`WelcomeMessage`/`LobbyUpdate`/`PlayerState`). Needs: a name-entry step client-side before/at connecting, a name field threaded through to the other client (e.g. added to `LobbyUpdate` or `PlayerState`), and swapping the hardcoded label for the received name. Low value, but cheap. |
