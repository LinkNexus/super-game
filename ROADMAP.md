...
| L21 | **Scale enemy/boss difficulty to player count** | Enemy count/aggression and boss health/attack patterns are currently fixed regardless of 1 vs. 2 active players — with co-op live, 2 players trivialize the current tuning. Scale these with active player count so co-op stays challenging; also a good place to pass for general gameplay-feel improvements now that both solo and co-op are playable end to end. |
| L22 | **Harden server against malformed client input** | `server/src/main.cpp`'s `.message` handler does `nlohmann::json::parse(message).get<shared::PlayerInput>()` with no try/catch — a malformed payload (bad JSON, missing/mistyped field) throws an uncaught `nlohmann::json` exception, which would crash the whole server process and drop every active game, not just the offending connection. Wrap the parse+deserialize in try/catch and just ignore/log bad messages per-connection instead. |
